...
class StatisticsTest extends TestCase
{
    ...
    public function testHitsShow()
    {
         ...
         */
        $this->get('api/statistics/link/'.$link->id, $header)->assertStatus(204);

        factory(Hit::class, 4)->create(['link_id' => $link->id]);

        /**
         * Assert that we see a status of 200 and the
         * hits created by the Hits factory.
         */
        $this->get('api/statistics/link/'.$link->id, $header)->assertStatus(200)->assertJsonStructure([
                'hits' => [
                    '*' => [
                        'id',
                        'link_id',
         ...
         */
        $this->get('api/statistics/link/non_existing_id', $header)->assertStatus(404);
    }
}
